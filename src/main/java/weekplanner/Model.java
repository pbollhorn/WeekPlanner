...
public class Model {
	...
	// "jdbc:postgresql://16.16.155.85:5432/WeekPlannerDB2";
	...
	public static void moveTaskDown(int listNumber, int taskNumber) {
		...
		try {
			...
			// Return immediately if taskNumber is >= count-1, because then task cannot be
			// moved further down
	...
	public static void moveTaskUp(int listNumber, int taskNumber) {
		...
		// Return immediately if taskNumber is 0, because then task cannot be moved
		// further up
		if (taskNumber == 0)
			return;
	...
	public static void saveTask(int listNumber, int taskNumber, String taskDescription) {
		...
		try {
			...
			// Update task
			sql = "UPDATE task SET description='" + taskDescription + "' WHERE number=" + taskNumber + " AND list_id="
					+ list_id;
			System.out.println(sql);
			statement.execute(sql);
